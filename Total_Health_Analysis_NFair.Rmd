...
count_matches <- function(x, y, z) {
  data%>%
        filter(disflag == x & sexflag == y & tothealthflag == z) %>%
...
total <- c(sum(results$good),sum(results$poor))
...
results$gpercent <- results$good/sum_row(results$good,results$poor)
results$ppercent <- results$poor/sum_row(results$good,results$poor)
```
...
write.table(totalresults, file = "totalnf_health.csv", sep = ",", row.names = F)
...
write.table(totalchi, file = "totalnf_chi.csv", sep = ",", row.names = F)
```
