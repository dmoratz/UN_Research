...
#Record observations of each combination for needed and received welfare
results <- foreach(i = rep(1,4), j = rep(c(0,1),2), z = c(0,0,1,1), .combine = 'rbind')%do% {
  count_matches(i,j,z, column = "welflag")
}

results <- as.data.frame(cbind(results[1:2,1],results[3:4,1]))
colnames(results) <- c("nwel","wel")

#Total and subtotal individuals w & w/o disabilities
total <- c(sum(results$nwel),sum(results$wel))
...
results$nwelpercent <- results$nwel/sum_row(results$nwel,results$wel)
results$welpercent <- results$wel/sum_row(results$nwel,results$wel)
...
nwelci <- foreach(i = results[,"nwelpercent"], j = rep(1.96, nrow(results)), k = rowSums(results[,1:2]), .combine = 'rbind') %do% {
  conint(i,j,k)
}
colnames(nwelci) <- c("nwellower95", "nwelupper95")

welci <- foreach(i = results[,"welpercent"], j = rep(1.96, nrow(results)), k = rowSums(results[,1:2]), .combine = 'rbind') %do% {
  conint(i,j,k)
}
colnames(welci) <- c("wellower95", "welupper95")

results <- cbind(results,nwelci,welci)
...
#Chi Square for Men vs Women needed and received welfare

welchi <- chisq.test(results[1:2,1:2], correct = FALSE)$p.value
...
results$grouptot <- rowSums(results[,c("nwel","wel")])
...
#Add a Group Name, Reorder Variables, Save Country Chi
nepalchi <- as.data.frame(cbind(welchi))
...
#Add a Group Name, Reorder Variables, Save Country Chi
swazichi <- as.data.frame(cbind(welchi))
...
#Add a Group Name, Reorder Variables, Save Country Chi
leschi <- as.data.frame(cbind(welchi))
...
#Add a Group Name, Reorder Variables, Save Country Chi
mozchi <- as.data.frame(cbind(welchi))
...
#Add a Group Name, Reorder Variables, Save Country Chi
zamchi <- as.data.frame(cbind(welchi))
...
#Add a Group Name, Reorder Variables, Save Country Chi
malchi <- as.data.frame(cbind(welchi))
...
write.table(totalresults, file = "wel_analysis.csv", sep = ",", row.names = F)
...
write.table(totalchi, file = "wel_chi.csv", sep = ",", row.names = F)
```
...
ggplot(data = totalresults, aes(x = group, y = welpercent, group = country)) +
  geom_point(aes(color = country)) +
  geom_errorbar(ymin = totalresults$wellower95, ymax = totalresults$welupper95)+
  labs(title = "Rate of Self Reported\n Needed and Received by Group", x = "Groupings of Individuals", y = "Percent Reporting\n Needed and Received Welfare", color = "Country") +
